...
/**
 * Publishes the fixed brand blue the breathing selection pulse uses for its
 * stroke and glow onto :root, so the CSS keyframes never hardcode a value.
 * RAW, not COLORS — the pulse must look identical in both themes, like the
 * card and die artwork.
 */
