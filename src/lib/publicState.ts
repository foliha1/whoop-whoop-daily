...
export interface PublicState {
  ...
  // Frozen seat map — host's authoritative pid → seat mapping. Joiners
  // learn their own seat by looking themselves up here.
  seatMap: Array<{ seat: number; pid: string; display_name: string }>;
  ...
  // the same room does not collide with the first game's rows.
  gameId: string;
  // Seats whose pid is no longer in room presence, or whose heartbeat
