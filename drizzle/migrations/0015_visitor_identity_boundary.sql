...
BEGIN
  FOREACH f IN ARRAY ARRAY[
    'caller_visitor(text)','whoop_points_rows()','whoop_points_for(text, date)','whoop_points_table(date)',
    'whoop_points_active_identities(date)','whoop_points_all(date)','whoop_score_table(integer, date)',
    'whoop_points_config()','whoop_points_apply_decay(integer, integer)','whoop_points_first_try(jsonb, integer, integer)',
    'whoop_points_game(jsonb, integer, integer, boolean)','whoop_points_tier(integer)','whoop_points_tier_floor(text)',
    'daily_puzzle_date(integer)','daily_season_start(integer)','gen_daily_group_code()','legacy_links_frozen()',
    'session_email()','request_ip()',
    'daily_result_reject_reason(integer, date, integer, integer, jsonb, integer, date)',
    'classic_result_reject_reason(timestamp with time zone, timestamp with time zone, integer, jsonb, integer, integer, integer, boolean)',
    'get_daily_event_counts(integer)',
    'create_daily_group(text, text, text)','join_daily_group(text, text, text)','leave_daily_group(uuid, text)',
    'get_my_groups(text, integer)','get_group_today(uuid, integer, text)','get_group_season(uuid, integer, text)',
  ...
  FOREACH f IN ARRAY ARRAY['admin_classic(date, date)','admin_retention_split()','admin_signin_funnel(integer)']
  LOOP
